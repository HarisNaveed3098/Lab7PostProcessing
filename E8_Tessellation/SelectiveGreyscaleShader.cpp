...
SelectiveGreyscaleShader::SelectiveGreyscaleShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"texture_vs.cso", L"selective_greyscale_ps.cso");
}

SelectiveGreyscaleShader::~SelectiveGreyscaleShader()
{
	if (screenSizeBuffer)
	{
...
void SelectiveGreyscaleShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void SelectiveGreyscaleShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix,
	const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture,
	float width, float height, float mouseX, float mouseY, float radius, float feather)
{
	HRESULT result;
	...
	result = deviceContext->Map(mouseBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	mousePtr = (MouseBufferType*)mappedResource.pData;
	mousePtr->mousePos = XMFLOAT2(mouseX, mouseY);
	mousePtr->radius = radius;
	mousePtr->feather = feather;
	deviceContext->Unmap(mouseBuffer, 0);
	deviceContext->PSSetConstantBuffers(1, 1, &mouseBuffer);
